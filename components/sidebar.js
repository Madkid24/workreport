      ...
      <div className={`hidden md:flex md:w-64 h-screen ${isDarkMode ? 'bg-gray-800' : 'bg-[#ecf0f3]'} text-gray-500 flex flex-col shadow-lg`}>
        ...
        <nav className="flex-1 mt-6">
          <ul className="space-y-6">
            ...
            <li><a href="#" onClick={showModal} className="flex items-center px-4 py-2 ml-8 hover:bg-gray-200 text-xl font-sans"><FontAwesomeIcon icon={faInfoCircle} className="w-6 h-6" /><span className="ml-2">Instructions</span></a></li>
      ...
      {isModalVisible && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className={`p-6 md:p-8 rounded-lg shadow-lg w-[90%] md:w-[800px] max-h-[90vh] overflow-y-auto ${isDarkMode ? 'bg-gray-800 text-gray-200' : 'bg-white text-black'}`}>
            <h2 className={`text-xl md:text-2xl font-bold mb-4 md:mb-6 text-center ${isDarkMode ? 'text-gray-100' : 'text-black'}`}>Instructions</h2>
            <ul className="list-disc pl-5 md:pl-8 space-y-3 md:space-y-4 text-base md:text-lg leading-relaxed">
              <li>
              ...
              <li>
                <strong>Step 3:</strong> After clicking "Generate," you'll be taken to the editor where the questions will be displayed. Note: The download and close buttons will be disabled until all questions are generated.
              </li>
              <li>
            ...
            <div className="flex justify-end mt-4 md:mt-6">
              <button onClick={closeModal} className="bg-blue-500 text-white px-4 md:px-6 py-2 md:py-3 rounded-lg shadow hover:bg-blue-600">
                Close
              </button>
            </div>
          </div>
